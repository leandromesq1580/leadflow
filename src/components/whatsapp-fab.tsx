 ...
 */
export async function WhatsAppFab({ phone, message, label }: Props) {
  const locale = await getLocale()
  const L = (pt: string, en: string, es: string) => locale === 'en' ? en : locale === 'es' ? es : pt
  const msg = message || L('Olá! Gostaria de saber mais sobre o Lead4Pro.', "Hello! I'd like to know more about Lead4Pro.", '¡Hola! Me gustaría saber más sobre Lead4Pro.')
  const lbl = label || L('Fale Conosco!', 'Chat with us!', '¡Escríbenos!')
  const cleanPhone = phone.replace(/\D/g, '')
  const href = `https://wa.me/${cleanPhone}?text=${encodeURIComponent(msg)}`
  ...
  return (
    ...
    >
      ...
      <span className="text-[13px] sm:text-[14px] font-bold whitespace-nowrap relative">{lbl}</span>
