...
import { leadCanAccess } from '@/lib/crm-access'
import { UpsellGate } from '@/components/upsell-gate'

/** pathname → metadados da feature pro upsell (mesmo set do appointment-gate). */
const FEATURE_MAP = (L: (pt: string, en: string, es: string) => string): { prefix: string; feature: string; icon: string; tagline: string }[] => [
  ...
  { prefix: '/dashboard/appointments', feature: L('Agenda', 'Calendar', 'Agenda'), icon: '📅', tagline: L('Marque e gerencie reuniões com seus leads, com lembretes automáticos.', 'Book and manage meetings with your leads, with automatic reminders.', 'Agenda y gestiona reuniones con tus leads, con recordatorios automáticos.') },
  { prefix: '/dashboard/pipeline', feature: L('Funil de vendas', 'Sales pipeline', 'Flujo de ventas'), icon: '📋', tagline: L('Arraste cada lead por estágio — do primeiro contato até o fechamento.', 'Drag each lead through every stage — from first contact to close.', 'Arrastra cada lead por cada etapa: del primer contacto al cierre.') },
  ...
  { prefix: '/dashboard/templates', feature: L('Modelos', 'Templates', 'Plantillas'), icon: '📝', tagline: L('Mensagens prontas que convertem, com variáveis automáticas.', 'Ready-made messages that convert, with automatic variables.', 'Mensajes listos que convierten, con variables automáticas.') },
  { prefix: '/dashboard/automations', feature: L('Automações', 'Automations', 'Automatizaciones'), icon: '⚡', tagline: L('Acompanhamentos que disparam sozinhos no momento certo.', 'Follow-ups that fire on their own at the right time.', 'Seguimientos que se activan solos en el momento justo.') },
...
interface Props {
  /** true quando o buyer é lead-only (vem do layout server). */
  active: boolean
  children: React.ReactNode
}

export function LeadGate({ active, children }: Props) {
  const pathname = usePathname()
  const t = useT()
  const L = (pt: string, en: string, es: string) => t._locale === 'en' ? en : t._locale === 'es' ? es : pt

  if (!active) return <>{children}</>
  if (leadCanAccess(pathname)) return <>{children}</>
