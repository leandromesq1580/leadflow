...
export function ClientsInbox() {
  ...
  return (
    <div className="rounded-2xl overflow-hidden flex" style={{ background: 'var(--bg-card)', border: '1px solid #e8ecf4', height: 'calc(100vh - 230px)', minHeight: 480 }}>
      {/* Lista */}
      <div className="w-[320px] flex flex-col" style={{ borderRight: '1px solid #e8ecf4' }}>
        <div className="px-4 py-3" style={{ borderBottom: '1px solid #e8ecf4' }}>
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Buscar cliente…"
            className="w-full px-3 py-2 rounded-lg text-[12px] focus:outline-none" style={{ background: 'var(--bg-soft)', border: '1px solid #e8ecf4' }} />
        </div>
          ...
          {filtered.length === 0 ? (
            ...
            return (
              <button key={c.buyer_id} onClick={() => setSel(c.buyer_id)}
                ...
                style={{ background: active ? 'var(--accent-light)' : 'transparent', borderBottom: '1px solid #f1f5f9', borderLeft: active ? '3px solid #6366f1' : '3px solid transparent' }}>
          ...
          <>
            <div className="px-5 py-3 flex items-center gap-3" style={{ borderBottom: '1px solid #e8ecf4' }}>
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-2 text-[11px]" style={{ color: 'var(--fg-secondary)' }}>
                  ...
                  <span className="font-bold px-1.5 py-0.5 rounded" style={{ background: 'var(--accent-light)', color: '#6366f1' }}>{PLAN_LABEL[current.crm_plan] || current.crm_plan}</span>
                </div>
              </div>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-2" style={{ background: 'var(--bg)' }}>
              ...
              {msgs.map(m => (
                <div key={m.id} className={`flex ${m.direction === 'out' ? 'justify-end' : 'justify-start'}`}>
                  <div className="max-w-[78%] px-3 py-2 rounded-2xl" style={{ background: m.direction === 'out' ? '#dcf8c6' : 'var(--bg-card)', border: m.direction === 'in' ? '1px solid #e8ecf4' : 'none' }}>
            ...
            <div className="p-3 flex gap-2 items-end" style={{ borderTop: '1px solid #e8ecf4' }}>
              <textarea value={text} onChange={e => setText(e.target.value)} rows={1}
                ...
                placeholder="Responder o cliente… (Enter envia)" className="flex-1 px-3 py-2 rounded-xl text-[13px] resize-none focus:outline-none" style={{ background: 'var(--bg)', border: '1px solid #e8ecf4', maxHeight: 100 }} />
