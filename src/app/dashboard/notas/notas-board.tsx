...
export function NotasBoard() {
  ...
  if (blocks === null) {
    return <div className="text-[13px] py-16 text-center" style={{ color: '#94a3b8' }}>{L('Carregando…', 'Loading…', 'Cargando…')}</div>
  }
    ...
    <div>
      ...
      {blocks.length === 0 ? (
        <div className="rounded-2xl p-10 text-center" style={{ background: '#fff', border: '1px dashed #cbd5e1' }}>
          <p className="text-[14px] font-semibold" style={{ color: '#475569' }}>{L('Nenhum bloco ainda', 'No blocks yet', 'Ningún bloque todavía')}</p>
          <p className="text-[12px] mt-1" style={{ color: '#94a3b8' }}>{L('Clique em “+ Novo bloco” pra criar a checklist de licença de uma pessoa.', 'Click “+ New block” to create someone\'s license checklist.', 'Haz clic en “+ Nuevo bloque” para crear el checklist de licencia de una persona.')}</p>
        </div>
      ) : (
