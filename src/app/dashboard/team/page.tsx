    ...
    <div className="p-6 max-w-4xl mx-auto">
      ...
      <div className="rounded-2xl p-5 mb-6" style={{ background: 'var(--bg)', border: '1px solid var(--border)' }}>
        ...
        <div className="flex gap-3">
          <button onClick={() => updateMode('auto_roundrobin')}
            ...
            style={{
              background: mode === 'auto_roundrobin' ? 'var(--accent)' : 'var(--bg-card)',
              color: mode === 'auto_roundrobin' ? '#fff' : 'var(--fg-secondary)',
              border: `1px solid ${mode === 'auto_roundrobin' ? 'var(--accent)' : 'var(--border)'}`,
            }}>
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        <div className="rounded-xl p-4 text-center" style={{ background: 'var(--bg)', border: '1px solid var(--border)' }}>
          <p className="text-[24px] font-extrabold" style={{ color: 'var(--accent)' }}>{members.length}</p>
      ...
      {/* Add Member Modal */}
      {showAdd && (
        <div className="rounded-2xl p-6 mb-6" style={{ background: 'var(--bg-card)', border: '2px solid var(--accent)', boxShadow: '0 4px 20px rgba(124,58,237,0.1)' }}>
      ...
      {/* Members List */}
      {members.length === 0 ? (
        <div className="rounded-2xl p-10 text-center" style={{ background: 'var(--bg)', border: '1px solid var(--border)' }}>
        ...
        <div className="space-y-3">
          {members.map(m => (
            <div key={m.id} className="rounded-xl p-4"
              style={{ background: 'var(--bg-card)', border: editingId === m.id ? '2px solid var(--accent)' : '1px solid var(--border)', opacity: m.is_active ? 1 : 0.5 }}>
              ...
              {editingId === m.id ? (
                ...
                <div className="flex items-center gap-4">
                  <div className="w-10 h-10 rounded-full flex items-center justify-center text-white text-[14px] font-bold"
                    style={{ background: m.is_active ? 'var(--accent)' : '#94a3b8' }}>
                  ...
                  <div className="text-center px-3">
                    <p className="text-[18px] font-extrabold" style={{ color: 'var(--accent)' }}>{m.leads_count}</p>
