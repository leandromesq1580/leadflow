...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { type, quantity, note, leadLanguage } = await request.json()
  ...
  if (type !== 'appointment' && !isLeadLanguage(leadLanguage)) {
    return NextResponse.json({ error: 'Selecione BR ou espanhol para adicionar créditos.' }, { status: 400 })
  }

  const { data, error } = await adminDb.from('credits').insert({
    buyer_id: id,
    type,
    lead_language: type === 'appointment' ? 'pt' : leadLanguage,
